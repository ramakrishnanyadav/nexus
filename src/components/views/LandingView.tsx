...
export function LandingView({ onStart }: { onStart: () => void }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="text-center max-w-3xl flex flex-col items-center">
      ...
      <button onClick={onStart} className="px-8 py-4 bg-[#4F46E5] text-white rounded-full font-bold text-lg hover:bg-[#4338ca] transition-all flex items-center gap-2 shadow-lg shadow-[#4F46E5]/20">
